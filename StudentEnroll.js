...
function enrollStudent(subject, studentName) {
  ...
  alert(`${studentName} enrolled in ${subject}`); // Fixed template literal syntax
}
...
function unenrollStudent(subject) {
  ...
  let nameToUnenroll = prompt(`Enter the name of the student to unenroll from ${subject}:`); // Fixed template literal syntax

  let index = studentArray.indexOf(nameToUnenroll);
  if (index > -1) {
    studentArray.splice(index, 1);
    alert(`${nameToUnenroll} unenrolled from ${subject}`); // Fixed template literal syntax
  } else {
    alert(`${nameToUnenroll} is not enrolled in ${subject}`); // Fixed template literal syntax
  }
}
  ...
  while (true) {
    ...
    while (true) { // Inner loop for operations
      ...
      if (selectedOperation === "Enroll") {
        let studentName = prompt(`Enter the student's name to enroll in ${selectedSubject}:`); // Fixed template literal syntax
